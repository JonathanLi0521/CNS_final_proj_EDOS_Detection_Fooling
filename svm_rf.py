...
best_svm = joblib.load("./svm/best_svm.model")
print("load best svm model")
# Evaluation
...
def evaluate(y_test, y_pred, cm_title='Confusion matrix', display_labels=attack_name, return_fool_ratio=True, count = 0):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(f"./svm_rf/fig{count}.png")
...
best_rf_adv = joblib.load("./rf/best_rf_adv.model") # trained rf adv model (trained by rf detection model)
print("load best rf adv model")
...
best_svm_retrain = joblib.load("./svm/best_svm_retrain.model") # trained retrain svm detection model
print("load best svm retrain")
